...
export class MsgStorageService {
  ...
  async saveMsg (msg: any) {
    ...
    try {
      ...
      // Save to Hot Cache (Memory)
      this.cache.put(msgData.msg_id, msgData);

      // Save to Cold Storage (DB)
      await this.msgRepo.save(msgData);

      // Cache in Redis (expire in 1 hour) - keep for compatibility if configured
